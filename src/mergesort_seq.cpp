...
int main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; i < sequences.size() - 1; i+=2) {
        next_level[0].push_back(mergeFiles(sequences[i], sequences[i + 1], MAX_MEMORY));
    }
    size_t current_level = 1;
    while (next_level.back().size() > 1) {
        next_level.push_back({});
        for (size_t i = 0; i < next_level[current_level - 1].size() - 1; i += 2) {
            next_level[current_level].push_back(mergeFiles(next_level[current_level - 1][i], next_level[current_level - 1][i + 1], MAX_MEMORY));
        }
